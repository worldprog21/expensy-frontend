...
import NewPaymentMethod from "./new"
...
const PaymentMethods = () => {
    ...
    const fetchData = async (page, filterField, filterValue) => {
        ...
        url = process.env.NEXT_PUBLIC_BASE_URL + `/payment-methods?pagination[page]=${page}&pagination[pageSize]=10&sort[0]=id%3Adesc` + filterQuery + userQuery
    ...
    const { isLoading, error, data, isFetching } =
        useQuery({
            queryKey: ["payment-methods", pagination.page, filterField, filterValue, user],
    ...
    const { mutate } = useMutation({
        mutationFn: (id) => {
            setDeleting(true);
            return axios.delete(
                process.env.NEXT_PUBLIC_BASE_URL + `/payment-methods/${id}`, {
                headers: {
                    'Authorization': 'Bearer ' + jwt
                }
            }
                            ...
                            <table className="divide-y divide-gray-300 min-w-full" >
                                ...
                                <thead className="bg-white">
                                    <tr>
                                        <th
                                            scope="col"
                                            className="whitespace-nowrap py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-0 w-full"
                                        >
                                            Name
                                        </th>
                                        <th
