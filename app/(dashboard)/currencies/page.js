...
const Currencies = () => {
    ...
    const fetchData = async (page, filterField, filterValue) => {
        ...
        url = process.env.NEXT_PUBLIC_BASE_URL + `/currencies?pagination[page]=${page}&pagination[pageSize]=10&sort[0]=id%3Adesc` + filterQuery + userQuery
    ...
    const { isLoading, error, data, isFetching } =
        useQuery({
            queryKey: ["currencies", pagination.page, filterField, filterValue, user],
    ...
    const { mutate } = useMutation({
        mutationFn: (id) => {
            setDeleting(true);
            return axios.delete(
                process.env.NEXT_PUBLIC_BASE_URL + `/currencies/${id}`, {
                headers: {
                    'Authorization': 'Bearer ' + jwt
                }
            }
        ...
        <div className="h-full -mt-6 p-10 md:p-5">
            <div className="rounded bg-gray-50 h-full border border-dashed p-5">
                <div className="flex items-center justify-between">
                    <h1 className="text-lg font-medium">Currencies</h1>
